# Ticketrue — Submission Pack

Prepared for **Build Week Hackathon Vol.2**. Use this file to copy verified links into the submission form. Do not replace a `TBD` item with an unverified value.

## Copy-ready submission details

| Submission field | Value |
| --- | --- |
| Project name | Ticketrue |
| One-line description | A BOT Chain concert-ticket dApp with on-chain ownership, public ticket verification, live wallet passes, and fair resale capped at the paid price. |
| Live website | https://ticketrue.web.id |
| GitHub repository | https://github.com/NaufalHabibiee/ticketrue |
| Testnet contract | [`0x324348D3f63850e77672da7e0E4A9806336eA210`](https://scan.bohr.life/address/0x324348D3f63850e77672da7e0E4A9806336eA210) |
| Testnet deployment | [`0x09fd283b1dbf9d4db6b6f5cf42315f2128d7cfc9b9bd50e043a2e83e3bfd57cc`](https://scan.bohr.life/tx/0x09fd283b1dbf9d4db6b6f5cf42315f2128d7cfc9b9bd50e043a2e83e3bfd57cc) · block `24509734` |
| On-chain event publish | [`0xa8e1a47c627935743a7d16a286c91060597f08b6af43a820bc33a7752a85d07e`](https://scan.bohr.life/tx/0xa8e1a47c627935743a7d16a286c91060597f08b6af43a820bc33a7752a85d07e) |
| On-chain ticket purchase | [`0x6a3ae3ae306d28227739dda7f9e7078195eab06d7bfe4b9de1783640a1b6650b`](https://scan.bohr.life/tx/0x6a3ae3ae306d28227739dda7f9e7078195eab06d7bfe4b9de1783640a1b6650b) · Ticket ID `1` |
| Mainnet contract | [`0x80CB3e83478fddC62617EfDe390FD73C927808B1`](https://scan.botchain.ai/address/0x80CB3e83478fddC62617EfDe390FD73C927808B1) |
| Mainnet launch announcement | https://ticketrue.web.id/#/launch |
| Dedicated Ticketrue X account | `TBD` |
| Main project X post tagging @BOTChain_ai | `TBD` |

## Evidence already collected

- [x] Live custom domain on GitHub Pages.
- [x] Solidity contract in `contracts/Ticketrue.sol`.
- [x] English README with a Deployment section and Testnet address.
- [x] Testnet contract deployment confirmed on BOT Chain Explorer.
- [x] Concert published on-chain.
- [x] Ticket purchase confirmed in MetaMask and recorded on-chain.
- [x] Ticket ID `1` shows a QR pass, public verification link, and downloadable PDF.
- [x] BOT Chain name and links are visible in the website footer.
- [x] Mainnet deployment, address, and explorer link.
- [x] Mainnet launch page published on the project website.
- [ ] Dedicated X account, five valid posts, and one project post tagging `@BOTChain_ai`.

## Mainnet handoff

After the deployment, add the following values here and send them to the project maintainer:

```text
Mainnet contract address: 0x80CB3e83478fddC62617EfDe390FD73C927808B1
Mainnet deployment block: 24354955
Mainnet deployment transaction: 0x122dcf1783425fb0dffd88d0dc70fdcbf4cf16417636b653fa3eb92c5206a043
Mainnet event publish transaction: TBD
Mainnet ticket purchase transaction: TBD
```

The mainnet release checklist is in [MAINNET_RELEASE.md](./MAINNET_RELEASE.md).

## Suggested proof screenshots

1. Ticketrue homepage on `ticketrue.web.id` with the BOT Chain footer.
2. MetaMask confirmation for the testnet ticket purchase.
3. Ticket ID `1` page showing “Recorded on BOT Chain”, the transaction hash, and QR pass.
4. BOT Chain Explorer pages for the deployed contract, event publish transaction, and ticket purchase transaction.
5. The X project post that tags `@BOTChain_ai`.
