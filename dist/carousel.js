export function mountCarousel(viewport){
  ...
  function schedule(){
    clearTimeout(timer);
    if(!disposed&&!paused&&!reduced.matches&&!document.hidden&&!hovered&&!focused&&visible)timer=setTimeout(()=>move(1),6000);
  }
  ...
  function togglePause(){paused=!paused;pauseButton.setAttribute('aria-pressed',String(paused));pauseButton.setAttribute('aria-label',paused?'Play carousel':'Pause carousel');pauseButton.textContent=paused?'▶':'Ⅱ';schedule()}
