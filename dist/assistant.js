...
const CHIPS={
  en:['How do I buy a ticket?','Which concerts are on?','How does resale work?','How do I connect my wallet?'],
  id:['Bagaimana cara beli tiket?','Konser apa saja yang ada?','Bagaimana cara resale?','Bagaimana cara connect wallet?']
};
...
const TOPICS=[
 ...
 {k:['seat selection','seat','seats','tier','tiers','category','cat 1','cat 2','vip','zone','zona','kursi','kategori','tipe tiket','ticket type','ticket types'],
  en:(c)=>'There are two kinds of shows. **Seat selection** shows let you pick a seat on the venue map (three zones, three prices). **Tier** shows let you pick a ticket category such as Festival, CAT 1 or VIP. '+(c.seated.length?`Seated: ${c.seated.map(e=>e.title).join(', ')}.`:'')  ,
 ...
 {k:['cancel listing','resale ticket','buy resale','tiket resale','beli resale','resell','resale','re-sell','sell my ticket','sell ticket','sell','scalp','scalper','calo','jual ulang','jual tiket','menjual','jual','marketplace'],
...
export function mountAssistant(getCtx){
  ...
  const ICON='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5h16a1 1 0 0 1 1 1v9a1 1 0 0 1-1 1H9l-4.5 3.5V16H4a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1Z"/><path d="M8 9.5h8M8 12.5h5"/></svg>';
  const CLOSE='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18"/></svg>';
  const fab=document.createElement('button');fab.type='button';fab.className='as-fab';fab.setAttribute('aria-label','Open Ticketrue assistant');fab.setAttribute('aria-expanded','false');fab.setAttribute('aria-controls','as-panel');fab.innerHTML=ICON+'<span class="as-fab-tip">Ask Ticketrue</span>';
  const panel=document.createElement('section');panel.id='as-panel';panel.className='as-panel';panel.setAttribute('role','dialog');panel.setAttribute('aria-label','Ticketrue assistant');panel.hidden=true;
  panel.innerHTML='<header class="as-head"><span class="as-avatar" aria-hidden="true">'+ICON+'</span><div><strong>Ticketrue Assistant</strong><small>Ask about concerts, tickets, wallet &amp; resale</small></div><button type="button" class="as-x" aria-label="Close assistant">'+CLOSE+'</button></header><div class="as-log" role="log" aria-live="polite"></div><div class="as-chips"></div><form class="as-form"><label class="as-sr" for="as-input">Message</label><input id="as-input" maxlength="300" autocomplete="off" placeholder="Ask about Ticketrue…"><button class="as-send" aria-label="Send message"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 12 20 4l-6 16-2.5-6.5L4 12Z"/></svg></button></form><p class="as-fine">Automated assistant. Answers only about Ticketrue.</p>';
  ...
  function open(){
    panel.hidden=false;fab.setAttribute('aria-expanded','true');fab.classList.add('open');fab.innerHTML=CLOSE;fab.setAttribute('aria-label','Close Ticketrue assistant');
    if(!started){started=true;add('bot',fmt(lang==='id'?'Halo! Aku asisten Ticketrue. Tanyakan soal konser, cara beli tiket, wallet, atau resale.':'Hi! I’m the Ticketrue assistant. Ask me about concerts, buying tickets, your wallet or resale.'));setChips()}
    setTimeout(()=>input.focus(),50);
  }
  function close(){panel.hidden=true;fab.setAttribute('aria-expanded','false');fab.classList.remove('open');fab.innerHTML=ICON+'<span class="as-fab-tip">Ask Ticketrue</span>';fab.setAttribute('aria-label','Open Ticketrue assistant');fab.focus()}
